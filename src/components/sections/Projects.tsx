        ...
        return (
            <section id="projects" className="py-24 relative overflow-hidden">
                <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
                    <div className="mb-16">
                        <h2 className="text-4xl md:text-5xl font-bold font-heading mb-4 text-slate-900 dark:text-white">Featured Projects</h2>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                        {/* Placeholder/Loading state content if needed, but returning full structure is better for SEO/Layout Shift */}
                    </div>
                </div>
            </section>
        );
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                    {projects.map((project, idx) => {
                        ...
                        if (typeof project.image === 'string') {
                            ...
                            displayImage = currentTheme === 'light' ? project.image.light : project.image.dark;
                        }
                        ...
                        return (
                            <motion.div
                                ...
                                className="group relative rounded-[2rem] overflow-hidden glass border border-black/5 dark:border-white/5 hover:border-black/20 dark:hover:border-white/20 transition-all duration-500 will-change-transform"
                            >
                                <div className={`w-full h-72 relative overflow-hidden flex items-center justify-center p-8`}>
                                    {displayImage.startsWith('bg-') ? (
                                        <div className={`absolute inset-0 ${displayImage}`} />
                                    ) : (
                                        <Image
                                            src={displayImage}
                                            alt={project.title}
                                    ...
                                    <div className="absolute inset-0 opacity-20 group-hover:opacity-40 transition-opacity duration-700 mix-blend-overlay flex items-center justify-center">
                                        <div className="w-32 h-32 rounded-full border border-black/50 dark:border-white/50 animate-[spin_10s_linear_infinite]" />
                                        <div className="w-48 h-48 rounded-full border border-black/30 dark:border-white/30 absolute animate-[spin_15s_linear_infinite_reverse]" />
                                    </div>
                                    ...
                                    >
                                        <a href={project.liveUrl} target="_blank" className="p-4 bg-slate-900 text-white dark:bg-white dark:text-black rounded-full hover:scale-110 transition-transform shadow-xl">
                                            <ExternalLink className="w-6 h-6" />
                                        </a>
                                        <a href={project.githubUrl} target="_blank" className="p-4 bg-white/50 dark:bg-black/50 text-slate-900 dark:text-white backdrop-blur-md rounded-full hover:scale-110 transition-transform shadow-xl border border-black/20 dark:border-white/20">
                                ...
                                <div className="p-8 md:p-10 relative">
                                    ...
                                    <h3 className="text-2xl md:text-3xl font-bold font-heading mb-4 text-slate-900 dark:text-white group-hover:text-primary transition-colors relative z-10">
                                        {project.title}
                                    </h3>
                                    ...
                                    <div className="flex flex-wrap gap-2 relative z-10">
                                        {project.tags.map(tag => (
                                            <span key={tag} className="text-sm font-medium px-4 py-1.5 glass rounded-full text-slate-900 dark:text-white tracking-wide">
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
